...
def PlotGraduatedData( x, ylist_ordered, range, figsize=None, default_ax=None, cmap='cividis', loc='lower right', title='Generic title', llabel='Least', rlabel='Most' ):
    '''
    PlotgraduatedData: 
    ...
    scaled_index = np.linspace( 0., 1., len( ylist_ordered ) )
    ...
    scaled_index = np.linspace( range[0], range[1], 100 )
    x, _ = np.meshgrid( scaled_index, np.arange( 3 ) )
    ax_ins.imshow( x, cmap=cmap )
    ax_ins.set_yticks( [] )
...
def PlotLabeledSegmentsImage( img, segment_dictionary, x_grid=None, y_grid=None, alpha=1., default_ax=None, figsize=None, color_map='tab10' ):
    '''
    ...
    assert color_map in allowed_cmaps, f'Incompatible color map. Allowed valies are {allowed_cmaps}. '  
    seg_idx = []
    segments = []
    ...
    im = default_ax.pcolormesh( x_grid, y_grid, img, cmap=cmap, norm=norm, alpha=alpha )
    cbar = plt.colorbar( im, ax=default_ax, fraction=0.046, pad=0.05, ticks=np.arange( len( segments ) ) )
